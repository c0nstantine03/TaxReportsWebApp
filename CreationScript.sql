...
DELIMITER $$
USE `taxReports`$$
CREATE DEFINER = CURRENT_USER TRIGGER `taxReports`.`report_list_AFTER_INSERT` AFTER INSERT ON `report_list` FOR EACH ROW
BEGIN
INSERT INTO report_history (`code`, content, author_id, inspector_id, status_id)
VALUES (NEW.`code`, NEW.content, NEW.author_id, NEW.inspector_id, NEW.status_id);
END$$

USE `taxReports`$$
CREATE DEFINER = CURRENT_USER TRIGGER `taxReports`.`report_list_AFTER_UPDATE` AFTER UPDATE ON `report_list` FOR EACH ROW
BEGIN
INSERT INTO report_history (`code`, content, author_id, inspector_id, status_id)
VALUES (NEW.`code`, NEW.content, NEW.author_id, NEW.inspector_id, NEW.status_id);
END$$
